<x-app-layout>
    ...
    <div class="container mx-auto p-8">
        ...
        <div class="bg-white shadow-xl rounded-lg p-8 mb-8">
            ...
            @if($team->yearlyTeamStats->isEmpty())
                    ...
                    <table class="min-w-full bg-white border border-gray-200 rounded-lg">
                        <thead class="bg-green-100 text-green-800">
                            <tr>
                                ...
                                <th class="py-3 px-4 border-b text-left">打率 (最新)</th>
                                <th class="py-3 px-4 border-b text-left">HR (最新)</th>
                                <th class="py-3 px-4 border-b text-left">打点 (最新)</th>
                        ...
                        <tbody>
                            @foreach ($team->players as $player)
                                <tr class="hover:bg-gray-50 {{ $loop->even ? 'bg-gray-50' : '' }}">
                                    ...
                                    <td class="py-3 px-4 border-b">
                                        {{-- 最新の打撃成績を取得（yearlyBattingStatsが降順でソートされているので最初の要素が最新） --}}
                                        @php
                                            $latestBattingStat = $player->yearlyBattingStats->first();
                                        @endphp
                                        @if($latestBattingStat)
                                            {{ number_format($latestBattingStat->batting_average, 3) }}
                                        @else
                                            -.-
                                        @endif
                                    </td>
